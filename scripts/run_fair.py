...
def fair_process(emissions, config_list, natural_ems, start_year, end_year, parallel_processing, WORKERS):
    updated_config = []
    for i, cfg in enumerate(config_list):
        updated_config.append({})
        for key, value in cfg.items():
            updated_config[i][key] = np.asarray(value) if isinstance(value, list) else value
...
def main():
    ...
    natural_ems_df = pd.read_csv("../input/fair-1.6.2-ar6/fair_wg3_natural_ch4_n2o.csv", sep=",")
    start_year = natural_ems_df.loc[0, "year"]
    end_year = 2100
    inter_start_year = 1850
    ...
    scens_to_run = [x for x in os.listdir(scen_file_dir) if x.endswith('.SCEN')]
    ...
    check_prehist = np.loadtxt('../input/fair-1.6.2-ar6/ssp245_constant-2020-ch4.csv', delimiter=',')
    ...
    for scenario in tqdm(scenarios, position=0, leave=True):
        ...
        (
            ...
            _, _
        ) = fair_process(emissions_in[scenario], config_list, natural_ems, start_year, end_year, parallel_processing, WORKERS)

    for scenario in scenarios:
        for var in ['temperatures']:
            mkdir_p(outdir.format(version, var.replace("effective_radiative_forcing", "erf")))
            df_out = pd.DataFrame(
                results_out[scenario][var][inter_start_year - start_year: end_year + 1 - start_year, :]
            )
            df_out['year'] = np.arange(inter_start_year, end_year + 1)
            df_out.set_index('year', inplace=True)
            df_out.to_csv(
                outdir.format(version, var.replace("effective_radiative_forcing", "erf")) + '{}.csv'.format(scenario),
                float_format="%6.4f"
            )
